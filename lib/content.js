...
export function getOutreachContent() {
  const filePath = path.join(CONTENT_DIR, "outreach.json");
  if (!fs.existsSync(filePath)) return { students: [] };
  const raw = fs.readFileSync(filePath, "utf-8");
  return JSON.parse(raw);
}

export function getSchoolSignupsContent() {
  const filePath = path.join(CONTENT_DIR, "school-signups.json");
...
export function getFeedbackContent() {
  const filePath = path.join(CONTENT_DIR, "feedback.json");
  if (!fs.existsSync(filePath)) return { photoEntries: [], submissions: [] };
  const raw = fs.readFileSync(filePath, "utf-8");
  const content = JSON.parse(raw);
  return {
    photoEntries: Array.isArray(content?.photoEntries) ? content.photoEntries : [],
    submissions: Array.isArray(content?.submissions) ? content.submissions : []
  };
}
